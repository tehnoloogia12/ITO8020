...
def getdata():
  ...
  sql="""select id,Asutus,Stryksus,Allstrukt,Ametikoht,Eesnimi,Perenimi,arvestuslikpalk,aasta from maintable"""
  # id,Asutus,Stryksus,Allstrukt,Ametikoht,Eesnimi,Perenimi,arvestuslikpalk
  # kui olid olemas key, op ja val, siis ehita where tingimus
  if key and val:
    if not op: # vaikimisi olgu op =
      op = "="
    # kontrollime, kas parameetrid ok  
    if (not (key in ["id","Asutus","Stryksus","Allstrukt","Ametikoht","Eesnimi","Perenimi","arvestuslikpalk","aasta"]) 
        or
        not (op in ["=","<",">","!="])):
